...
describe('Recintos do Zoologico', () => {
    ...
    test('Deve rejeitar quantidade não inteira', () => {
        const resultado = new RecintosZoo().analisaRecintos('MACACO', 2.5);
        expect(resultado.erro).toBe("Quantidade inválida");
        expect(resultado.recintosViaveis).toBeFalsy();
    });
    ...
    test('Deve encontrar recintos para 1 hipopótamo', () => {
        ...
        expect(resultado.recintosViaveis[2]).toBe('Recinto 5 (espaço livre: 1 total: 9)');
        expect(resultado.recintosViaveis.length).toBe(3);
    });

    test('Deve encontrar recinto com espaço exato para 2 leões', () => {
        const resultado = new RecintosZoo().analisaRecintos('LEAO', 2);
        expect(resultado.erro).toBeFalsy();
        expect(resultado.recintosViaveis[0]).toContain('Recinto 5 (espaço livre: 0 total: 9)');
        expect(resultado.recintosViaveis.length).toBe(1);
    });
...
describe('Recintos do Zoologico', () => {
    ...
    test('Deve encontrar recintos para 3 hipopótamos', () => {
    ...
    test('Não deve encontrar recintos para 1 leão em recinto de rio', () => {
        const resultado = new RecintosZoo().analisaRecintos('LEAO', 1);
        expect(resultado.erro).toBeFalsy();
        expect(resultado.recintosViaveis).not.toContain('Recinto 4 (espaço livre: 8 total: 8)');
    });
    ...
    test('Deve rejeitar quantidade zero de animal', () => {
        const resultado = new RecintosZoo().analisaRecintos('CROCODILO', 0);
        expect(resultado.erro).toBe("Quantidade inválida");
        expect(resultado.recintosViaveis).toBeFalsy();
    });

    test('Não deve encontrar recintos para 5 macacos', () => {
        const resultado = new RecintosZoo().analisaRecintos('MACACO', 5);
        expect(resultado.erro).toBe("Não há recinto viável");
        expect(resultado.recintosViaveis).toBeFalsy();
    });

    test('Não deve encontrar recintos para 1 hipopótamo devido a espaço ocupado', () => {
        const resultado = new RecintosZoo().analisaRecintos('HIPOPOTAMO', 1);
        expect(resultado.erro).toBeFalsy();
        expect(resultado.recintosViaveis).not.toContain('Recinto 1 (espaço livre: 0 total: 10)');
